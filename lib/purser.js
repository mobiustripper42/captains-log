...
export async function handleSlash({ db, chatId, captain, body }) {
  if (captain.role !== 'owner') {
    return { reply: `Slash commands are owner-only for now, ${captain.name}. Text Eric if something's off.` };
  }
  ...
  if (FILE_PATTERN.test(body)) {
    ...
    if (!pending.drafted_issue_body) {
      return { reply: `That draft looks empty, ${captain.name}. Re-send /feedback with the observation.` };
    }
    // drafted_issue_body is "title\n\nbody"; an empty body half is acceptable
    // (gh allows --body ""). Title is always non-empty by createPending's contract.
