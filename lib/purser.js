...
export const SLASH_COMMAND_PATTERN = /^\/(feedback|file)(?:\s|$)/i;
...
export async function routeParsed({
  ...
  if (intent === 'drill') {
    return {
      reply: `Drill capture is coming in 3.4, ${captain.name}. For now, hold the drill details — I'll get them next.`,
      state: null,
    };
  }
...
export async function handle({ chatId, body, source, db }) {
  ...
  // Real impl lands in 3.5 (#35).
  if (SLASH_COMMAND_PATTERN.test(body)) {
    return { reply: `Slash commands are coming in 3.5, ${captain.name}.` };
  }
